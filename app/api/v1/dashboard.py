...
def _category_snapshot(
    ...
    default_status_name: str, asof: datetime,
) -> dict[int, StatusCategory]:
    """Reconstructs each ticket's status category as of a past moment, using the transition
    activity log (falling back to the project's default/initial status for tickets with no
    ...
    for ticket in all_tickets:
        ...
        if ticket.deleted_at is not None and ticket.deleted_at <= asof:
            continue
        name = last_status_name[ticket.id][1] if ticket.id in last_status_name else default_status_name
...
def get_dashboard(
    ...
    snapshot = (
        _category_snapshot(all_tickets, activities, status_category_by_name, default_status.name, week_ago)
